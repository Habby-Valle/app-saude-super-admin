...
export function ClinicForm({
  ...
  async function handleFormSubmit(values: ClinicFormValues) {
    await onSubmit(values, logoFile, selectedPlanId)
  }
          ...
          {logoPreview ? (
            <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded-lg border">
              ...
              <button
                type="button"
                onClick={removeLogo}
                className="text-destructive-foreground absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-destructive shadow"
              >
                <X className="h-3 w-3" />
              </button>
            </div>
