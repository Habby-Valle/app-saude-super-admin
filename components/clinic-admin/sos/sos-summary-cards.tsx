...
interface ClinicSosSummaryCardsProps {
  active: number
  acknowledged: number
  resolvedToday: number
}

export function ClinicSosSummaryCards({
  active,
  acknowledged,
  resolvedToday,
}: ClinicSosSummaryCardsProps) {
  return (
    <div className="grid gap-4 sm:grid-cols-3">
      <Card className={active > 0 ? "border-destructive/50 bg-destructive/5" : ""}>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          ...
          <AlertTriangle
            className={`h-4 w-4 ${active > 0 ? "text-destructive" : "text-muted-foreground"}`}
          />
        </CardHeader>
        <CardContent>
...
export function ClinicSosSummaryCardsSkeleton() {
  return (
    <div className="grid gap-4 sm:grid-cols-3">
